...
export default function BottomNav({ currentRoute }: BottomNavProps) {
  ...
  return (
    <View className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 py-4 px-6 flex-row justify-between pb-8 z-20">
      <TouchableOpacity className="items-center" onPress={() => currentRoute !== '/' && router.replace('/' as any)}>
        <Home size={24} color={getIconColor('/')} />
        <Text className={`${getTextColor('/')} font-inter text-[10px] mt-1`}>Dashboard</Text>
      </TouchableOpacity>
      
      <TouchableOpacity className="items-center" onPress={() => currentRoute !== '/inventory' && router.replace('/inventory' as any)}>
      ...
      <TouchableOpacity className="items-center" onPress={() => currentRoute !== '/transactions' && router.replace('/transactions' as any)}>
      ...
      <TouchableOpacity className="items-center" onPress={() => currentRoute !== '/reports' && router.replace('/reports' as any)}>
        <BarChart3 size={24} color={getIconColor('/reports')} />
        <Text className={`${getTextColor('/reports')} font-inter text-[10px] mt-1`}>Reports</Text>
      </TouchableOpacity>
      
      <TouchableOpacity className="items-center" onPress={() => currentRoute !== '/settings' && router.replace('/settings' as any)}>
